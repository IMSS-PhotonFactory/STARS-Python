...
class StarsInterface(stars.StarsInterface):
    ...
    def __init__(self, nodename, srvhost, keyfile = '', srvport = stars.StarsInterface.DEFAULT_PORT):
        ...
        if(hasattr(self,'_mode')==False):
            self._mode = {}
    #
        self._conectCheckinterval = 0.01
        self._chktimebase = 0
    ...
    #---------------------------------------------------------------
    # Connect Check in Mainloop
    #---------------------------------------------------------------
#   try, Later issue -> # for making sure 20221027
#    def chkconnect(self):
#        _csts = 'Ok:'
#        if(dc.isconnected() == False):
#            destsendstr="Terminate STARS %s. [Device disconnection]\n" %(self.nodename)
#            _outputlog(WARN, destsendstr)
#            rt=self.send('System _Msg '+destsendstr)
...
#   <- try, Later issue
    ...
    def Mainloop(self, inthandler=None, intervaltime = ''):
        ...
        timebase = time.time()
        _cktimebase = timebase
        while(self._termsignal==False):
            #Check existing no socket
#   try, Later issue -> # for making sure 20221027
#            if( self._conectCheckinterval > 0):
#                _cknow = time.time()
#                _ckti = _cknow - _cktimebase
#                if(_ckti > self._conectCheckinterval):
#                    self.chkconnect()
#                _cktimebase = time.time
#   <- try, Later issue
            if(len(readfds) <= 0):
